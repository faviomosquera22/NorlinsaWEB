...
export type ShareholderBalance = { shareholder: string; availableBalance: number; paidGuides: number; logisticsServices: number; withdrawals: number };
...
const demoBalances: BalanceSnapshot = {
  isDemo: true,
  balances: [{ shareholder: "Daniel", availableBalance: 3660, paidGuides: 340, logisticsServices: 0, withdrawals: 0 }, { shareholder: "Jorge", availableBalance: 8040, paidGuides: 1560, logisticsServices: 720, withdrawals: 400 }, { shareholder: "Favio", availableBalance: 2500, paidGuides: 0, logisticsServices: 0, withdrawals: 0 }],
...
type BalanceRow = { shareholder_name: string; available_balance: number; paid_guides: number; logistics_services: number; withdrawals: number };
...
const labels: Record<string, string> = { opening_balance: "Saldo inicial", adjustment_credit: "Ajuste a favor", adjustment_debit: "Ajuste en contra", withdrawal: "Retiro", guide_payment: "Pago de guía", guide_reserve: "Guía registrada", logistics_service: "Servicio logístico" };
...
export async function getBalanceSnapshot(): Promise<BalanceSnapshot> {
  ...
  const [balancesResult, entriesResult] = await Promise.all([
    supabase.from("shareholder_available_balances").select("shareholder_name,available_balance,paid_guides,logistics_services,withdrawals").eq("organization_id", membership.organization_id).order("shareholder_name"),
  ...
  return {
    isDemo: false,
    balances: ((balancesResult.data ?? []) as BalanceRow[]).map((row) => ({ shareholder: row.shareholder_name, availableBalance: Number(row.available_balance), paidGuides: Number(row.paid_guides), logisticsServices: Number(row.logistics_services), withdrawals: Number(row.withdrawals) })),
